...
/**
 * Client-side signup logic with enhanced safety checks for TypeScript.
 */
async function signUpClientSide(values: SignupFormValues): Promise<FormState> {
    ...
    // Safety check to resolve TS2345/TS2769 errors
    if (!auth || !db) {
        console.error('[email-form.tsx] Firebase services are not available.');
        return { success: false, message: "Service temporarily unavailable. Please refresh and try again.", timestamp: Date.now() };
    }
    ...
    try {
        ...
        while (!isCodeUnique && attempts < 5) {
            ...
            const qCheck = query(collection(db, 'users'), where('referralCode', '==', newReferralCode), limit(1));
            const existingCodeQuery = await getDocs(qCheck);
        ...
        if (error.code?.startsWith('auth/')) {
            let authMessage = "There was a problem creating your account. Please check your details and try again.";
            if (error.message) {
        ...
        return { success: false, message: 'Something went wrong on our end. Please try again later.', timestamp: Date.now() };
    }
}
...
function ReferralDisplay({ code }: { code: string }) {
    ...
    return (
        <div className="mt-6 w-full text-center p-4 border-2 border-dashed border-primary/50 rounded-lg bg-primary/10">
            ...
            <p className="text-muted-foreground mt-1 text-sm">Share this code with your friends. For every 10 users who sign up with your code, you'll get another month of the elite plan for free!</p>
...
export function EmailForm() {
    ...
    // Critical fix for "Application Error" / Hydration Mismatch
    useEffect(() => {
        setIsMounted(true);
    ...
    // Don't render until client-side hydration is complete to prevent crashes
